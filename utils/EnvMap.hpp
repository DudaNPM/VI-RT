...
struct HDRImageBuffer {
   ...
   * \param y column of the pixel
   */
  void update_pixel(const OurRGB& s, size_t x, size_t y) {
    // assert(0 <= x && x < w);
    // assert(0 <= y && y < h);
    data[x + y * w] = s;
  }
   ...
   */
  void update_pixel(const OurRGB& s, size_t x, size_t y, float r) {
    // assert(0 <= x && x < w);
    // assert(0 <= y && y < h);
   ...
   * Clear image buffer.
   */
  void clear() { std::memset(&data[0], 0, w * h * sizeof(OurRGB)); }

  size_t w; ///< width
  size_t h; ///< height
  std::vector<OurRGB> data; ///< pixel buffer
