...
bool Perspective::GenerateRay(const int x, const int y, Ray *r, const float *cam_jitter) {
    ...
    float xc = xs * tanf(fovW / 2.0f);
    float yc = ys * tanf(fovH / 2.0f);

    r->o =  Eye;
    r->dir = Vector(c2w[0][0] * xc  + c2w[0][1] * yc + c2w[0][2],
                    c2w[1][0] * xc  + c2w[1][1] * yc + c2w[1][2],
                    c2w[2][0] * xc  + c2w[2][1] * yc + c2w[2][2]);
    r->dir.normalize();
